...
export const CycleVsMoodRule: InsightRule = {
  ...
  async run(userId: string): Promise<InsightResult | null> {
    ...
    const avgMoodMenstrual    = menstrualDays.reduce((s, r) => s + Number(r.avg_mood), 0) / menstrualDays.length;
    const avgMoodNonMenstrual = nonMenstrualDays.reduce((s, r) => s + Number(r.avg_mood), 0) / nonMenstrualDays.length;

    const diff = avgMoodMenstrual - avgMoodNonMenstrual;
    ...
    return {
      title: `Mood tends to be ${direction} during your period`,
      description: `Over the last 90 days, your average mood score was ${avgMoodMenstrual.toFixed(1)}/5 on menstrual days and ${avgMoodNonMenstrual.toFixed(1)}/5 on non-menstrual days.`,
      confidence: label,
      ...
      supportingData: {
        ...
        avg_mood_menstrual: avgMoodMenstrual.toFixed(2),
        avg_mood_non_menstrual: avgMoodNonMenstrual.toFixed(2),
